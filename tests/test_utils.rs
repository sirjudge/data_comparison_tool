...
pub fn setup() -> (config::Config, log::Log) {
    ...
    config.database_1_config.table_name = "a_testTable1".to_string();
    config.database_2_config.table_name = "a_testTable2".to_string();
...
pub async fn generate_mysql_table(table_name: &str, config: &config::Config) -> usize {
    ...
    let mysql_pool = mysql::get_connection("ComparisonData",&log, config).await;
