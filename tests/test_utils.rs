...
pub fn setup() -> (Config, Log) {
    let config = Config::new_from_file("tests/comp.toml");
...
pub async fn generate_testing_tables(config:&Config,log:&Log, table_name: &str, db_config: &DatabaseConfig) -> usize {
